...
# Explain: Assign feature('Responsive browser viewport', [('index.html','<meta name from the value shown here. Apply feature('Dialog overlay and readable controls', [('site.css','.care-dialog { width... in the document-building sequence. Apply feature('naloxones row typography and spacing', [('site.css','.places-list a {'),('site.... in the document-building sequence.
...
feature('naloxones row typography and spacing', [('site.css','.places-list a {'),('site.css','.places-list small {')], 'Venue links are bold; addresses, indoor hints and metadata appear on separate muted lines.', 'Helps someone identify the destination before reading the final directions.')

# Explain: Apply section('Navigation and ordinary interface sound') in the document-building sequence. Assign feature('Home tracker and About navigation', [('nav.js','const... from the value shown here. Assign feature('PulseGuard brand and active page', [('nav.js','name.te... from the value shown here.
...
feature('PulseGuard brand and active page', [('nav.js','name.textContent = "PulseGuard";'),('nav.js','a.setAttribute("aria-current", "page");')], 'The brand is inserted as text. The matching navigation link is marked as the current page for assistive technology.', 'Makes the service and the current location within it identifiable.')
...
feature('Page title and sharing description', [('index.html','<title>PulseGuard'),('index.html','<meta property="og:description"')], 'The title identifies the tab and the Open Graph description supplies sharing metadata.', 'Explains the purpose before someone opens the tracker.')
feature('Headline and purpose', [('index.html','<h1>Help for the moment'),('index.html','<p class="lede">PulseGuard looks')], 'The heading introduces the problem; the paragraph describes multiple signals, a response check and preparation of human help.', 'States the notice, verify and help sequence in plain language.')
...
feature('Embedded live tracker', [('index.html','<iframe title="PulseGuard'),('index.html','if (event.origin !== location.origin')], 'The homepage embeds fall-detection.html and permits its location, motion and audio capabilities. The message guard accepts size changes only from that same-origin frame.', 'Keeps the working tracker available from the homepage without trusting arbitrary window messages.')
...
feature('Complete tracker shutdown', [('app.js','function stopSensors()'),('app.js','window.addEventListener("pagehide", stopSensors);')], 'Shutdown cancels timers and sensor listeners, aborts outstanding naloxones work and clears the location watch. Leaving the page invokes it too.', 'Limits monitoring to the session the person is using.')

section('Live location satellite map and naloxones')
...
# Explain: Assign feature('Five nearest real mapped naloxones records', [('app.js','co... from the value shown here. Apply feature('Distance calculation', [('app.js','function metresBetween(a, b)'),('app.j... in the document-building sequence. Assign feature('Actual venue name and street address', [('app.js','co... from the value shown here.
feature('Five nearest real mapped naloxones records', [('app.js','const query = \'[out:json]'),('app.js','.sort((a, b) => a.distance')], 'Overpass finds naloxone records within ten kilometres. The sort and slice retain up to five, closest first by straight-line distance.', 'Reduces the search for nearby equipment without substituting example entries.')
...
feature('Actual venue name and street address', [('app.js','const host = buildings.filter'),('app.js','const name = host?.tags?.name')], 'Only a mapped building containing the naloxones coordinate can supply its building name. Address, recorded name or coordinates provide fallbacks.', 'Makes the destination recognizable without assigning an unrelated nearby business.')
# Explain: Assign feature('Indoor directions kept beneath the venue', [('app.js'... from the value shown here. Assign feature('Batched building lookup and temporary cache', [('app.... from the value shown here. Assign feature('naloxones access hours and source limitations', [('app.js',... from the value shown here.
feature('Indoor directions kept beneath the venue', [('app.js','const notes = [tags["naloxone:location"]'),('app.js','locationDetail.textContent =')], 'Recorded room, floor or restroom directions remain separate from the venue title and are inserted as plain text.', 'Helps someone navigate the final part of the search.')
feature('Batched building lookup and temporary cache', [('app.js','const venueQuery ='),('app.js','if (complete) naloxoneVenueCache.set')], 'One additional request fetches building outlines. Successful results remain in memory for ten minutes; failed lookups can be retried.', 'Reduces repeated waiting without discarding naloxones coordinates when names fail.')
feature('naloxones access hours and source limitations', [('app.js','detail.textContent = Math.round(item.distance)')], 'Each row shows distance, recorded naloxones access and naloxones hours. Missing values stay not recorded; building hours are not substituted.', 'Avoids claiming equipment is accessible merely because a venue is named.')
# Explain: Assign feature('Directions to the precise naloxones pin', [('app.js','link.... from the value shown here. Assign feature('Refresh after movement and ignore old results', [('ap... from the value shown here. Apply section('Trusted contact and prepared message') in the document-building sequence.
feature('Directions to the precise naloxones pin', [('app.js','link.href = "https://www.google.com/maps/dir/'),('app.js','naloxoneMarkers.push(L.marker')], 'The Google Maps link uses the naloxones coordinates, while the satellite marker uses those same coordinates and the resolved place label.', 'Keeps directions tied to the equipment instead of moving the destination to a building centre.')
feature('Refresh after movement and ignore old results', [('app.js','if ((!naloxoneOrigin && Date.now() - naloxoneAttemptAt'),('app.js','if (request !== naloxoneRequest) return;',1)], 'The first fix or movement beyond 100 metres triggers a lookup. A request counter prevents older responses from replacing a newer search.', 'Keeps nearby resources aligned with the current location.')
...
feature('Message includes position and signals', [('app.js','const message = "PulseGuard:'),('app.js','const where = place && Date.now() - place.at <= 60000')], 'The prepared text describes the event and includes only a recent position; otherwise it says a recent position is unavailable.', 'Gives a responder context while making missing location explicit.')
# Explain: Assign feature('Human reviewed SMS and missing contact notice', [('ap... from the value shown here. Apply section('About page') in the document-building sequence. Apply feature('Current capabilities and limits', [('about.html','<h1>What PulseGuard can'... in the document-building sequence.
...
section('About page')
feature('Current capabilities and limits', [('about.html','<h1>What PulseGuard can'),('about.html','No automatic call to 911 and no responder network.')], 'The About page describes what the browser demonstrates and the capabilities it does not provide.', 'Supports informed use of the project rather than an unsupported guarantee.')
...
feature('Illustrative resource data and category filter', [('shelters.html','const SEED = ['),('shelters.html','rows = rows.filter(r => r.kind === kind);')], 'The page starts with example naloxones, shelter and hydrant entries. The selected category filters that local collection.', 'Demonstrates resource browsing; it is separate from live Overpass naloxones results.')
...
feature('Optional Google key example', [('config.example.js','window.PULSEGUARD_GOOGLE_MAPS_KEY =')], 'The file is an example key setting for an earlier integration. The current tracker obtains naloxones through Overpass and does not require this key.', 'Separates old setup material from the active map implementation.')
...
('0 to 15 seconds', 'PulseGuard helps people notice possible danger, check responsiveness, and reach human help. The homepage presents emergency choices, support links, and the tracker. About explains the method, sources, and privacy.', [('index.html','<h1>Help for the moment'),('about.html','<h1>What PulseGuard can')]),
...
def build():
    ...
    for mission,filename in [(False,'PulseGuard_Code_Presentation'),(True,'PulseGuard_Code_Mission_References')]:
        ...
        title='PulseGuard Website Features and Mission' if mission else 'PulseGuard Website Feature Code Guide'
